...
class C_ActPow_Price_DispMode_Dialog(QDialog): ## Classe Dialog Despacho Price da Potencia Ativa
    ...
    def acceptPrice(self):
        self.PriceParameters = {}
        self.PriceParameters["ChargeTrigger"] = self.ChargeTrigger_DoubleSpinBox.text().replace(",", ".")
        self.PriceParameters["DischargeTrigger"] = self.DischargeTrigger_DoubleSpinBox.text().replace(",", ".")
        if self.TimeTrigger_DoubleSpinBox.isEnabled():
            self.PriceParameters["TimeChargeTrigger"] = self.TimeTrigger_DoubleSpinBox.text().replace(",", ".")
