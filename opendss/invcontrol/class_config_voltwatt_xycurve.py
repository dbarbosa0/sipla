...
class C_Config_VoltWatt_XYCurve_Dialog(QDialog):
    def __init__(self):
        super().__init__()
    ...
    def InitUI(self):
        ...
        self.XYCurve_GroupBox = QGroupBox("voltwatt_curve")
        ...
        self.XYCurve_GroupBox_Label = QLabel(
            "Pontos X: Tensão (p.u.)\nPontos Y: Potência ativa disponível (p.u.)")
    ...
    def viewXYCurve(self):
        ...
        #Add Axis Labels
        self.graphWidget.setLabel('left', 'Potência ativa disponível (p.u.)', color='red', size=20)
    ...
    def Accept(self):
        ...
        if checkCont > 1:
            ...
            if self.checkXYCurve(Item.name, Item.getPointsX(), Item.getPointsY()):
                ...
                self.dataVW_XYCurve["XYCurveName"] = Item.name
                self.dataVW_XYCurve["npts"] = str(len(self.XYCurveXarray))
                self.dataVW_XYCurve["Xarray"] = self.XYCurveXarray
                self.dataVW_XYCurve["Yarray"] = self.XYCurveYarray
                self.close()
...
class Config_XYCurve_GroupBox_TreeWidget_Item(QTreeWidgetItem):
    def __init__(self, parent,  name, pointsX, pointsY, color):
        ...
        self.Config_XYCurve = C_Config_VoltWatt_XYCurve_Dialog()
        ## Column 0 - Text:
